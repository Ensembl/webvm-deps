package Website::Utilities::TimeSpan;
#########
# date manipulation
#
...
sub seed {
	...
	if($seed =~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/) {
	    ...
	    #
	    print STDERR qq(Website::Utilities::TimeSpan::seed incorrect format\n);
	    return undef;
	}
	$self->{'seed'} = $seed;
    }
...
sub day_of_week {
  my ($self) = @_;
  my ($year, $month, $day, $hour, $min, $sec) = $self->{'seed'} =~ /^([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})$/;
  my $time = timegm($sec, $min, $hour, $day, $month-1, $year-1900);
  return (qw(Sun Mon Tue Wed Thu Fri Sat))[(gmtime($time))[6]];
}
