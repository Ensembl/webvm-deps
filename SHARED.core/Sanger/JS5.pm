...
use constant 'INFO'  => 0;
use constant 'WARN'  => 1000;
use constant 'ERROR' => 2000;
use constant 'FATAL' => 9999;

sub _new {
  my $class    = shift;
  my $self = {
    '_log_level'     => ERROR,
    '_log_verbosity' => 'normal',
    '_log_prefix'    => ''
  };
}

sub log_prefix :lvalue {
  $_[0]{'_log_prefix'}; 
}

sub log_level :lvalue {
  $_[0]{'_log_level'};
}
...
sub _warning { 
  my $self     = shift;
  my $severity = shift;
  return if $self->log_level < $severity;
  my $t = $Carp::CarpLevel;
  $Carp::CarpLevel = 1;
  $self->_log_verbosity eq 'normal' ? carp( $self->log_prefix, @_ ) : cluck( $self->log_prefix, @_ );
  $Carp::CarpLevel = $t;
}

sub info {
  my $self = shift;
  $self->_warning( INFO, "[INFO]", @_ );
}

sub warning {
  my $self = shift;
  $self->_warning( WARN, "[WARN]", @_ );
}

sub error {
  my $self = shift;
  $self->_warning( ERROR, "[ERROR]", @_ );
}

sub fatal {
  my $self = shift;
  $self->_warning( FATAL, "[FATAL]", @_ );
}

sub new {
  my ($class, $refs) = @_;
  my $self = { '_log_level' => 0 };
  bless $self, $class;
  $self->handler($refs);
  return $self;
}
