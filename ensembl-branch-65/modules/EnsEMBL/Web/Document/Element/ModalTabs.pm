# $Id: ModalTabs.pm,v 1.6 2011-11-17 13:36:48 sb23 Exp $
...
sub init {
  ...
  foreach (@{$self->entries}) {
    if (($type eq 'Config' && $_->{'id'} eq $config) || ($type eq 'UserData' && $_->{'id'} eq 'user_data') || ($type eq 'UserConfig' && $_->{'id'} eq 'manage_cfg')) {
      $_->{'class'} = 'active';
